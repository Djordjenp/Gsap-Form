...
containers.forEach(container => {
    ...
    input.addEventListener('focus', () => {
        ...
        if (!input.value){
            tl.fromTo(line, {attr: {d: start}}, {attr: {d: end}, ease: 'Power2.easeOut', duration: .75} )
            tl.to(line, {attr: {d: start}, ease: "elastic.out(3, 0.5)"}, "<50%")
            //Placeholder shift
            tl.to(placeholder, {top: -15, left: 0, scale: .7, duration: .5, ease: 'Power2.easeOut'}, '<15%')
        }
    })
...
//Checking email validation and phone
...
gsap.fromTo('#eye', {scaleY: 1}, {scaleY: .3, repeat: -1, yoyo: true, repeatDelay: 0.5, ease: 'Power2.easeOut'})
